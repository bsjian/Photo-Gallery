(function(window){
	...
	BoshengAlbum.prototype.init = function() {
     ...
     */
	BoshengAlbum.prototype.getAlbumElement = function() {
		var tmp,
			ratio;
		if (document.getElementsByClassName) {
			if (document.getElementsByClassName('bosheng-album')[0]) {
				contentElement = document.getElementsByClassName('bosheng-album')[0];
				width = parseInt(window.getComputedStyle(contentElement,null).width);
			}
		}
		if (document.querySelectorAll) {
			if (document.querySelectorAll('.bosheng-album img').length !== 0) {
				tmp = document.querySelectorAll('.bosheng-album img');
			}
		}
		for (var i = 0; i < tmp.length; i++) {
			imgArray[i] = tmp[i].getAttribute('src');
			imgLink[i] = tmp[i].getAttribute('href');
			imgSize[i] = {
	 ...
	 * @returns {Object}
     */
	BoshengAlbum.prototype.getLayout = function () {
		if (usageLayout) {
			return usageLayout;
		} else {
			switch (layout) {
	...
	BoshengAlbum.prototype.addImage = function(image) {
     ...
     */
	BoshengAlbum.prototype.addClass = function (element,value) {
		var className,
			newClass = '';
     ...
     */
	BoshengAlbum.prototype.removeClass = function (element,value) {
		var className;
		if (element.getAttribute('class')){
     ...
     */
	BoshengAlbum.prototype.insertAt = function (parent, child ,n ) {
	 ...
	 * 创建遮罩层，并且为遮罩层添加点击事件
	 */
	BoshengAlbum.prototype.createOverlay = function() {
		var that = this;
	 ...
	 */
	BoshengAlbum.prototype.addClickEvent = function(image,url) {
		var that = this,
			link,
			src;
		if (url) {
	...
	/**
	 * 瀑布布局的包装函数
	 * @param external 外部BoshengAlbum的this指针
	...
	var Puzzle = function(external,imageCount) {
		...
		Puzzle.prototype.puzzleCountHelp = function(countGroup,array,offset,aspectRatio) {
			...
			divTmp.setAttribute('href',imgLink[array*6+offset])
	...
	window.onload = function() {
		if (typeof window.boshengAlbum === 'undefined') {
			window.boshengAlbum = new BoshengAlbum();
		}
	};
}(window));